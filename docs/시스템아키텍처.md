...
flowchart TB
    subgraph Client["📱 Expo RN 앱 (iOS)"]
        UI["expo-router 화면"]
        Parse["extractVideoId()\n로컬 파싱"]
        Preview["oEmbed 미리보기\n(쿼터 0)"]
        Handoff["watch_videos 핸드오프\n(메인 재생)"]
        IFrame["IFrame 미리듣기\n(보조·인앱)"]
    end
...
```mermaid
flowchart LR
    Start["[유튜브에서 재생]"] --> Cfg{"config/app\nhandoffMode?"}
    Cfg -->|watch_videos| WV["watch_videos?video_ids=…\n당일 order순 ≤50곡"]
    WV --> TLGG["유튜브 앱/웹\nTLGG 연속 재생 + [저장]"]
    Cfg -->|first_video| FV["첫 곡만 유튜브\n+ 인앱 미리듣기 승격"]
