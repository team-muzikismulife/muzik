...
| **테마 = 내장 풀 + dateKey 해시(서버리스)** | `themes` 컬렉션 제거. **days 문서가 있으면 `days.themeText`가 정본**, 없을 때(곡 0개)만 클라 계산(`missionFor`). 클라 계산을 항상 믿으면 앱·Functions 배포 스큐로 화면과 기록이 갈린다(이슈 #29). |
