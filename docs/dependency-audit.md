...
| 경로 | 노출/판단 | 조치 |
|---|---|---|
| Expo Router + Navigation→query-string7→decode-uri-component0.2.2 (moderate, 상위4개 경고) | 웹 의존성에 포함된다. React Navigation 기본 getStateFromPath는 queryString.parse를 호출하지만, 현재 Expo Router는 자체 fork를 연결하고 URL.searchParams로 파싱한다. Router의 query-string 직접 사용은 stringify다. **현재 앱의 외부 링크에서 취약 decoder까지 도달함은 확인되지 않았다.** 잠재 영향은 브라우저 응답성 저하이며 서버 코드 실행/DB 누출로 확대 해석하지 않음 | [고친 버전0.5.0](https://github.com/advisories/GHSA-vcc3-ghjq-m6fr)은 ESM이고 기존7.x CJS 의존성과 다르므로 강제 override하지 않았다. 공개 모집 전 실제 번들/전체 링크 진입 경로와 malformed URL 회귀를 검증하고 잔여 위험을 리뷰해야 한다. Expo 지원 Router/Navigation 조합 업그레이드는 별도 검증 |
