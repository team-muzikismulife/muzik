...
PWA-02A 일상 흐름에 이어 PWA-02B 설치·업데이트, 운영자 신고 처리, 피드백/방문 지표를 구현했다. 실제 외부 서비스 연결은 마지막 단계다. 유료플랜/카드/도메인/옛 배포/실데이터/홍보 조작 없음.
...
- 방문/설치 안내/설치 요청 수락/standalone 실행을 서버 KST 날짜별 사용자당 한 번씩 구분한다. 실패는 핵심 행동을 막지 않으며 무한 재시도·외부 분석·새 추적 ID가 없다. 사용량은 공식 콘솔로 연결하며 잔여 한도나 설치 기기 수를 만들어 표시하지 않는다.

추가 migration은 `202609210003_operations.sql`. 세부 권한 부여·무료 일시정지·복구·업데이트/rollback 절차는 [운영 문서](pwa-operations.md)를 따른다.
