...
export function validateEnv(env, { release = false } = {}) {
...
const cli = process.argv[1]?.replaceAll("\\", "/").endsWith("/check-env.mjs");
if (cli) {
  const env = { ...loadEnv("production", process.cwd(), ""), ...process.env };
  console.log(validateEnv(env, { release: process.argv.includes("--release") }));
}
