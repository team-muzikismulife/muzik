...
import { registerTrack, updateTrack, deleteTrack, previewTrack } from '@/lib/api';
...
export function TrackForm({ roomId, nickname, initialUrl, editTrack }: Props) {
  ...
  const onChangeUrl = async (text: string) => {
    ...
    try {
      const m = isMockPreviewEnabled() ? await fetchVideoMeta(id) : await previewTrack(roomId, id);
      if (my !== reqId.current) return; // 더 최신 입력이 있으면 버린다
      const parsed = parseTitle(m.title, 'author' in m ? m.author : m.artist);
      ...
      setPreviewError(toMessage(error, 'registerTrack'));
    }
  };
  ...
  const submit = async () => {
    if (!canSubmit || !videoId || saving.current) return;
    saving.current = true;
    setSubmitting(true);
    try {
      ...
      if (editTrack) {
        ...
        savedDate = result.dateKey;
      } else {
        const result = await registerTrack({
          roomId,
          videoId,
          comment: comment.trim(),
          nickname,
      ...
      toast(editTrack ? '수정 내용을 저장했어요' : '서버에 등록했어요. 팀원을 초대해 함께 들어보세요.');
      router.replace({ pathname: '/room/[id]', params: { id: roomId, date: savedDate } });
    } catch (e: unknown) {
