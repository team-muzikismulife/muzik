...
const { chromium } = require('node:module').createRequire(require('node:path').resolve(__dirname, '../../../legacy/expo/package.json'))('playwright');
...
(async () => {
  ...
  try {
    ...
    await first.screenshot({ path: '../../tests/artifacts/beta-room-mobile.png', fullPage: true, animations: 'disabled' });
    ...
    await first.waitForFunction(() => Array.from(document.images).some(i => i.naturalWidth > 120), null, { timeout: 10000 });
    await first.screenshot({ path: '../../tests/artifacts/beta-playlist-mobile.png', fullPage: true, animations: 'disabled' });
    await first.setViewportSize({ width: 1280, height: 900 });
    assert(await first.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth));
    await first.screenshot({ path: '../../tests/artifacts/beta-playlist-desktop.png', fullPage: true });
    ...
    await first.screenshot({path:'../../tests/artifacts/beta-browser-failure.png',fullPage:true});
    throw e;
  } finally {
    await browser.close();
